...
async function createTalentUsers() {
  ...
  for (const talent of talents) {
    try {
      ...
      if (existingUser) {
        console.log(`⚠️  User already exists: ${email} -> ${talent.prenom} ${talent.nom}`);
        ...
        results.push({
          ...
          status: updateError ? 'LINK_ERROR' : 'LINKED_EXISTING'
        });
        continue;
      }
      
      // Create new auth user with simpler approach
  ...
  console.log('\n=== RESULTS TABLE ===');
  console.log('| Talent Name | Email | Status | Auth User ID | Error |');
  console.log('|-------------|-------|--------|--------------|-------|');
  
  results.forEach(result => {
    console.log(`| ${result.talentName} | ${result.email} | ${result.status} | ${result.auth_user_id} | ${result.error || 'None'} |`);
  });
  
  // Save results to file
  const fs = require('fs');
  fs.writeFileSync('talent_auth_results_fixed.json', JSON.stringify(results, null, 2));
  console.log('\nResults saved to talent_auth_results_fixed.json');
}
