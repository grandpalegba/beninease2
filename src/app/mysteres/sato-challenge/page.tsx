    ...
    <div className="min-h-screen bg-white text-[#1A1A1A] flex flex-col items-center p-4 font-sans overflow-x-hidden">
      ...
      <main className="w-full max-w-6xl flex flex-col items-center mt-12">
        ...
        <div className="w-full grid grid-cols-1 md:grid-cols-3 items-end justify-items-center gap-10 md:gap-4 min-h-[380px] md:min-h-[450px] relative mb-12">
          ...
          {/* OKPÈLÈ (VERTICAL) */}
          <div className="relative flex flex-col items-center justify-center h-full pt-16 shrink-0">
                  ...
                  {holes.map((hIdx) => (
                    <motion.div key={hIdx} exit={{ opacity: 0, scale: 0 }}
                      className={`absolute rounded-full bg-[#120604] shadow-[inset_0_8px_20px_rgba(0,0,0,0.9)] border border-white/5
                        ${hIdx === 0 ? 'top-[35%] left-[22%] w-10 h-10 md:w-16 md:h-16' : ''}
                        ${hIdx === 1 ? 'top-[28%] left-[60%] w-8 h-8 md:w-14 md:h-14' : ''}
          ...
          {/* AWALÉ (VERTICAL, RÉDUIT, COULEUR DE LA JARRE) */}
          <motion.div animate={isWrong ? { x: [-5, 5, -5, 5, 0] } : {}} 
                      className="mb-8 flex flex-col md:flex-row gap-2 p-4 rounded-[2.5rem] border-b-8 shadow-2xl relative shrink-0"
                      style={{ background: jarColor, borderColor: '#2a1a0a' }}>
            <div className="grid grid-cols-1 gap-3 pr-3 border-r-2 border-black/30 relative z-10">
                {[...Array(4)].map((_, idx) => (
            ...
            <div className="grid grid-cols-1 gap-3 pl-3 relative z-10">
                {[...Array(4)].map((_, idx) => (
            ...
            <div className="flex flex-col items-center py-10">
              ...
              <div className="w-full bg-white p-8 rounded-[2.5rem] border border-gray-100 shadow-xl mb-10">
                ...
                <div className="mt-8 pt-6 border-t border-gray-100 text-center">
                    <p className="text-lg font-semibold text-[#1A1A1A]">Le mystère ne fait que commencer.</p>
                    <p className="text-sm text-gray-500 italic mt-1">Le Sato protège votre esprit, mais votre quête doit se poursuivre pour découvrir d'autres secrets d'Ifá.</p>
                </div>
              </div>
