...
// --- COMPOSANTS VISUELS (Restauration Haute Fidélité et Matériaux Unifiés) ---

// Graine d'Okpele (Matériau Clay)
...
const OkpeleRitual = ({ activeSeeds }: { activeSeeds: number }) => (
  <div className="relative flex flex-col items-center scale-90">
    ...
    <div className="w-20 h-10 border-t-[1.5px] border-x-[1.5px] border-yellow-600/30 rounded-t-full absolute -top-6 left-1/2 -translate-x-1/2 z-0" />
...
// Sato Jar (Haut de jarre finalisé visuellement)
const SatoJar = ({ holesCount, isOver }: { holesCount: number[], isOver: boolean }) => (
  <div className={`relative w-64 h-80 md:w-72 md:h-96 shrink-0 transition-transform duration-500 ${isOver ? 'scale-105' : 'scale-100'}`}>
    {/* Haut de la Jarre - Structure multicouche (Code HTML fourni) */}
    <div className="absolute -top-1 left-1/2 -translate-x-1/2 w-36 h-9 bg-[#1a0a07] rounded-[50%] z-20 shadow-[inset_0_4px_8px_rgba(0,0,0,0.6)]" />
    <div className="absolute top-0 left-1/2 -translate-x-1/2 w-[148px] h-7 bg-[#3d1810] rounded-[50%] z-10 border-b border-black/20" />

    {/* Corps de la Jarre (Clay texture) */}
...
// Mini Awale (Support Clay unifié)
      ...
      <div key={col} className="grid grid-cols-1 gap-2">
        {[...Array(4)].map((_, i) => (
          <div key={i} className="w-8 h-8 bg-[#3d1810] rounded-full shadow-inner flex flex-wrap justify-center items-center p-1 gap-0.5">
            {seedsCount > (col * 4 + i) * 2 && <div className="w-2 h-2 rounded-full bg-wood shadow-inner" />}
            {seedsCount > (col * 4 + i) * 2 + 1 && <div className="w-2 h-2 rounded-full bg-wood shadow-inner" />}
          </div>
        ))}
      </div>
    ))}
...
export default function MysteresPage() {
  ...
  if (loading) return <div className="h-screen flex items-center justify-center bg-white"><div className="w-8 h-8 border-4 border-primary border-t-transparent rounded-full animate-spin" /></div>;

  return (
    <div className="h-screen w-screen bg-white overflow-hidden relative touch-none font-sans text-on-surface">
              ...
              <AnimatePresence initial={false}>
                {mysteres.map((m, idx) => idx === currentIndex && (
                  <motion.div
                    ...
                    whileTap={{ scale: 0.98 }}
                    initial={{ x: 300, opacity: 0, scale: 0.9 }} animate={{ x: 0, opacity: 1, scale: 1 }} exit={{ x: -300, opacity: 0, scale: 0.9 }}
                  ...
                  >
                    ...
                    <div className="h-[60%] w-full overflow-hidden pointer-events-none select-none">
                    ...
                    <div className="p-7 flex flex-col flex-1 pointer-events-none">
                      ...
                      <p className="text-[10px] font-bold text-primary mt-1 uppercase italic">{m.subtitle}</p>
              ...
              {!isFinished ? (
                !showExplanation ? (
                  <div className="text-center px-4">
                    <motion.h2 initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-xl font-bold mb-10 text-gray-700 leading-relaxed">
                    ...
                    <div className="grid grid-cols-1 gap-3">
                      {['a', 'b', 'c', 'd'].map((l) => currentQuestions[qIndex]?.[`choice_${l}`] && (
                        ...
                        >
                          <span className="w-8 h-8 rounded-lg bg-orange-50 flex items-center justify-center font-bold text-primary mr-4 uppercase">{l}</span>
                  ...
                  >
                    <p className="text-lg italic font-medium text-primary">"{currentQuestions[qIndex]?.explanation}"</p>
                    <p className="text-[10px] mt-8 uppercase tracking-widest font-black text-gray-300 animate-pulse">Toucher pour continuer</p>
                  </motion.div>
                )
              ) : (
                <div className="text-center px-4">
                  <h2 className="text-2xl font-black mb-4 uppercase text-primary">Secret Révélé</h2>
                  <div className="bg-white p-6 rounded-[2.5rem] text-left mb-6 space-y-4 border border-gray-50 shadow-inner max-h-[250px] overflow-y-auto no-scrollbar">
                    {explanations.map((exp, i) => <p key={i} className="text-sm text-gray-600 leading-relaxed"><span className="text-primary mr-2">✦</span> {exp}</p>)}
                  </div>
                  <button onClick={handleNextMystery} className="w-full py-5 bg-primary text-white rounded-full font-bold uppercase tracking-widest text-[10px] shadow-lg active:scale-95 transition-transform">Découvrir le suivant</button>
                </div>
              )}
            </div>
