...
export default function MysteresPage() {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        const { data: mData, error: mErr } = await supabase.from('mysteres').select('*');
        const { data: qData, error: qErr } = await supabase.from('questions').select('*');
        const { data: tData, error: tErr } = await supabase.from('themes').select('id, name');

        if (mErr || qErr || tErr) throw new Error("Erreur Supabase");
        ...
        toast.error("Connexion aux mystères impossible");
      } finally {
        setLoading(false);
      }
    }
      ...
      <AnimatePresence mode="wait">
        {view === "gallery" ? (
          <motion.div key="gallery" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="h-full flex flex-col items-center justify-center p-6">
            <div className="relative w-full max-w-[320px] h-[580px] flex items-center justify-center">
                ...
                {mysteres.map((m, idx) => (
                  idx === currentIndex && (
                    <motion.div
                      ...
                      onTap={(e, info) => {
                        // Ouvre le mystère sur le click (tap), si ce n'est pas un drag (seuil 5px)
                        if (Math.abs(info.offset.x) < 5) startRitual();
                      }}
                      initial={{ x: 300, opacity: 0 }} animate={{ x: 0, opacity: 1 }} exit={{ x: -300, opacity: 0 }}
                      transition={{ type: "spring", stiffness: 260, damping: 25 }}
                      className="absolute w-[320px] h-[580px] bg-white rounded-[40px] shadow-2xl overflow-hidden border-[6px] border-white cursor-grab active:cursor-grabbing flex flex-col"
                    >
                      <div className="pt-5 pb-3 px-7 text-center border-b border-gray-50">
                        <span className="text-[11px] font-medium text-gray-400 uppercase tracking-[0.35em]">
                          {themes[m.theme_id] || "Chargement..."}
                        </span>
                      </div>
                      ...
                      <div className="p-7 flex flex-col flex-1">
                        ...
                        <div className="mt-3 pt-3 border-t border-gray-50 flex-1 overflow-y-auto no-scrollbar">
                          {/* Mise en abyme restaurée sous le sous-titre */}
                          <p className="text-[15px] text-gray-400 italic leading-[1.6]">"{m.mise_en_abyme}"</p>
                        </div>
                        <div className="text-[9px] font-bold text-center text-gray-300 uppercase tracking-widest mt-2 border-t border-gray-50 pt-2">
          ...
          <motion.div
            ...
            // Swipe vers le bas pour revenir à la galerie
              ...
              {!isFinished ? (
                !showExplanation ? (
                      ...
                      {['a', 'b', 'c', 'd'].map((l) => (
                        currentQuestions[qIndex]?.[`choice_${l}`] && (
                          <motion.div key={l} drag dragSnapToOrigin
                            ...
                            className="p-4 bg-white border border-gray-100 rounded-2xl shadow-sm cursor-grab active:cursor-grabbing flex items-center group touch-none z-50 hover:border-orange-100 transition-colors"
                          >
                  ...
                  >
                    ...
                    <p className="text-[10px] mt-4 uppercase tracking-widest font-black text-gray-300">Touchez pour le secret suivant</p>
                  </motion.div>
                )
              ) : (
                <div className="text-center">
                  ...
                  <button onClick={() => setView("gallery")} className="w-full py-4 bg-[#a0412d] text-white rounded-full font-bold uppercase tracking-widest text-[10px] shadow-lg active:scale-95 transition-transform">Voir un autre secret</button>
                </div>
              )}
            </div>
          </motion.div>
