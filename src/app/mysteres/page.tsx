...
export default function MysteresPage() {
  ...
  const startRitual = () => {
    if (currentQuestions.length > 0) {
      ...
      toast.error("Mystère non disponible");
    }
  };
  ...
  return (
    <div className="h-screen w-screen bg-white overflow-hidden relative touch-none font-sans text-[#1A1A1A]">
      ...
      <AnimatePresence mode="wait">
        {view === "gallery" ? (
          <motion.div key="gallery" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="h-full flex items-center justify-center">
              ...
              <AnimatePresence initial={false}>
                {mysteres.map((m, idx) => idx === currentIndex && (
                  <motion.div
                    ...
                    onTap={(e, info) => { if (Math.abs(info.offset.x) < 5) startRitual(); }}
                  ...
                  >
                    <div className="pt-6 pb-2 text-center">
                      <span className="text-[10px] font-medium text-gray-400 uppercase tracking-[0.4em]">{themes[m.theme_id] || "Culture"}</span>
                    </div>
                    ...
                    <div className="p-7 flex flex-col flex-1">
                      <h2 className="text-[22px] font-black leading-tight uppercase">{m.title}</h2>
                      <p className="text-[10px] font-bold text-[#a0412d] mt-1 italic uppercase">{m.subtitle}</p>
          ...
          >
            <div className="w-full max-w-5xl flex flex-row items-center justify-center gap-10 h-[400px] shrink-0">
              ...
              {!isFinished ? (
                !showExplanation ? (
                  <div className="text-center">
                    <h2 className="text-lg font-bold mb-10 px-4 text-gray-700">{currentQuestions[qIndex]?.question}</h2>
                  ...
                  <div onClick={() => { setShowExplanation(false); setQIndex(p => p + 1); setTimeLeft(64); }} className="p-8 bg-orange-50 rounded-[2rem] text-center cursor-pointer">
                    <p className="text-lg italic font-medium text-[#a0412d]">"{currentQuestions[qIndex]?.explanation}"</p>
                    <p className="text-[10px] mt-4 uppercase tracking-widest font-black text-gray-300 underline">Continuer le rituel</p>
                  </div>
                )
              ) : (
                <div className="text-center">
                  <h2 className="text-2xl font-black mb-6 uppercase text-[#a0412d]">Secret Révélé</h2>
                  <div className="bg-white p-6 rounded-[2rem] text-left mb-6 border border-gray-50 shadow-inner space-y-3">
                  ...
                  <button onClick={() => setView("gallery")} className="w-full py-4 bg-[#a0412d] text-white rounded-full font-bold uppercase tracking-widest text-[10px]">Retourner à la galerie</button>
                </div>
              )}
            </div>
          </motion.div>
