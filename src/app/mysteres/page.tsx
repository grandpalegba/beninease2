  ...
  const handleDrop = (choiceLetter: string, info: any) => {
    if (info.point.y < 350) {
      ...
      if (choiceLetter === correct) {
        ...
        if (qIndex < 3) {
          toast.success("La jarre s'illumine...");
        ...
        handleFailure("La jarre rejette cette offrande...");
      }
    }
  };
      ...
      <AnimatePresence mode="wait">
        ...
        {view === "tresor" && (
          <motion.div 
            ...
            className="absolute inset-0 bg-[#1a0f0a] z-[60] flex flex-col items-center justify-center p-8 text-center text-white"
          >
            <h2 className="text-[#fdb813] font-black tracking-[0.4em] uppercase mb-8 text-xs">Trésor de Sagesse</h2>
            ...
            >
              <div className="absolute inset-0 bg-[#fdb813] rounded-full blur-3xl opacity-20 animate-pulse" />
              <img 
                src={currentM.treasure_image_url} 
            ...
            <p className="text-[#fdb813] italic mb-6">Trésor n°{currentM.mystere_number}</p>
            
            <div className="bg-white/5 border border-white/10 p-6 rounded-3xl text-sm leading-relaxed max-w-sm">
              <p className="text-gray-300">
                {currentM.inspiration || "Ce fragment d'histoire rejoint votre collection sacrée."}
              </p>
            </div>
            ...
            <button 
              ...
              className="mt-10 px-10 py-4 bg-[#fdb813] text-[#3d1810] rounded-full font-black shadow-xl uppercase tracking-widest text-xs"
            >
              Reprendre la Route
            </button>
          </motion.div>
        ...
        {view === "locked" && (
          <motion.div key="locked" className="absolute inset-0 bg-white z-[100] flex flex-col items-center justify-center p-8 text-center">
            <h2 className="text-2xl font-black text-[#a0412d] mb-4 uppercase">Jarre Scellée</h2>
            <p className="text-gray-500 text-sm mb-8">Utilisez le mot de pouvoir pour libérer vos graines.</p>
            <input 
                type="password" 
                value={password} 
                onChange={e => setPassword(e.target.value)}
                placeholder="Mot de Pouvoir" 
                className="w-full max-w-xs p-4 border rounded-full mb-4 text-center outline-none focus:border-[#a0412d]" 
            />
            <button 
                onClick={() => { if(password.toLowerCase() === "benin"){ setLives(8); setView("gallery"); }}} 
                className="w-full max-w-xs bg-[#a0412d] text-white p-4 rounded-full font-black"
            >
                Invoquer
            </button>
          </motion.div>
