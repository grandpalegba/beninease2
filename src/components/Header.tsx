...
const Header = ({ hideTop = false }: { hideTop?: boolean }) => {
  ...
  const isMainPage = ["/sagesses", "/savoirs", "/histoires", "/talents", "/tresors", "/visions", "/yonygames"].includes(pathname);
          ...
          {user ? (
            <div className="relative user-menu-container">
              <button onClick={() => setShowDropdown(!showDropdown)} className="flex items-center gap-2 group">
                <div className="relative h-10 w-10 rounded-full overflow-hidden border-2 border-white/20 group-hover:border-[#FCD116] transition-all">
                  {profile?.avatar_url ? (
                ...
                <ChevronDown size={14} className={cn("text-white/40 transition-transform", showDropdown && "rotate-180")} />
              </button>
